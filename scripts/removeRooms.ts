...
const deletedRooms = await deleteRooms(today.subtract({ hours: ROOM_RETENTION_DAYS * 24 })).catch(
	(err) => {
		logger.error('Error deleting rooms:', err);
	}
);
if (deletedRooms) {
	logger.info(`Deleted ${deletedRooms.length} rooms older than ${ROOM_RETENTION_DAYS} days`);
