...
async function scrapePuzzle() {
  ...
  const gameDataRaw = html.match(/window\.gameData\s*=\s*(.*)<\/script><\/div>/)
    ?.[1];
...
async function loadPuzzle() {
  ...
  if (todaysPuzzle !== null) {
    ...
    logger.info(
      `Scraped puzzle for ${puzzle.displayDate}: ${puzzle.centerLetter}_${
        puzzle.outerLetters.join("")
      }, ${puzzle.answers.length} answers, ${puzzle.pangrams.length} pangrams`,
    );
...
if (import.meta.main) await loadPuzzle();
